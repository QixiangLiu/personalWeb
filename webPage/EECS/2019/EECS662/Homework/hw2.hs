...
several new language constructs:
 ...
 - For functions, we introduce variables, function abstraction (i.e., lambda
   terms) and function application (i.e., function calls)

 - For sums, we introduce terms to create pairs "Left 1", "Right (1, 2)", and
   branching terms "case x of Left y -> y; Right z -> z + z" to eliminate pairs.
...
Values are a subset of terms, consisting of integer values, pairs of values,
functions, or sum values (either on the left or right).

data Value = VInt Int | VPair Value Value | VFun Name Term
...
Our first task is to implement substitution over our new term language.  This
builds on the definition of substitution we've been developing in class---in
particular, your implementation of substitution for function abstractions and
function applications should follow the definition we've given in class.

!!!The entirely new concern here is to do with sum constructors and branching.  Be
careful about your variables---branching introduces new variables.!!!

In the course of substituting a value into a term, you'll need to turn the value
back into a term.  On the whiteboard, we can rely on values being a subset of
terms, but in Haskell we need to make this transformation explicit.  This is the
role of the `termOf` function.
...
Case is used for Question2?
For example, a term in our extended language might look something like:

   (\x. case x of Left y -> y; Right z -> 3) (Right (1, 2))

which would be represented by the Haskell value:
Case Term Name Term Name Term ==> (Case (Var "x") "y" (Var "y") "z" (Const 3))
   (Lam "x" (Case (Var "x")
                 "y" (Var "y")
                 "z" (Const 3)))
...
termOf :: Value -> Term
termOf (VInt v) = Const v
termOf (VPair v1 v2) = Pair (termOf v1) (termOf v2)
termOf (VFun n t) = Lam n t
termOf (VLeft v) = InLeft (termOf v)
termOf (VRight v) = InRight (termOf v)

--termOf _ = error "Unimplemented"



subst :: Term -> Value -> Name -> Term
subst (Const x) _ _ = Const x
subst (Plus t1 t2) v n = (subst t1 v n) `Plus` (subst t2 v n)
subst (Times t1 t2) v n = (subst t1 v n) `Times` (subst t2 v n)
subst (Pair t1 t2) v n = Pair (subst t1 v n) (subst t2 v n)
subst (Fst t) v n = Fst (subst t v n)
subst (Snd t) v n = Snd (subst t v n)
subst (Var n1) v n2 = if n1 == n2
                      then (termOf v)
                      else (Var n1)
subst (Lam n1 t1) v n2 =
              if n1==n2
              then Lam n1 t1
              else Lam n1 (subst t1 v n2)
subst (App t1 t2) v n = App (subst t1 v n) (subst t2 v n)
subst (InLeft t) v n = InLeft (subst t v n)
subst (InRight t) v n = InRight (subst t v n)
...
{--
case Left 41 of Left x -> x; Right y -> y
41
eval $ parse ("case Left 41 of Left x -> x; Right y -> y")
Just (VInt 41)
(\x -> case Left 41 of Left x -> x; Right y -> y) 42
41
eval $ parse "(\\x. case Left 41 of Left x -> x; Right y -> y) 42"
Just (VInt 41)
--}
substTests1 = test [(subst (Const 4) (VInt 4) "x" ~?= Const 4),
                    (subst (Plus (Const 4) (Const 5)) (VInt 4) "x" ~?= Plus (Const 4) (Const 5)),
                    (subst (Var "x") (VInt 4) "x" ~?= Const 4),
                    (subst (Var "z") (VInt 4) "x" ~?= Var "z"),
                    (subst (InLeft (Var "x")) (VInt 42) ("x") ~?= InLeft (Const 42))
                    ]

substTests2 = test [ subst (parse term) value variable ~?= result | (term, value, variable, result) <- tests]
                where tests = [
    {--------------------------|         Term (syntax format)        |      Value        |  Variable |---------}
                               ("case z of Left x -> x; Right y -> y", (VRight (VInt 3)), "z",
                                    {-Yields-} (Case (InRight (Const 3)) "x" (Var "x") "y" (Var "y"))),

                               ("case Right z of Left z -> z; Right z -> z", (VInt 5), "z",
                                    {-Yields-} (Case (InRight (Const 5)) "z" (Var "z") "z" (Var "z"))),

                               ("(\\x. fst x)", (VPair (VInt 5) (VInt 3)), "x",
                                    {-Yields-} (Lam "x" (Fst (Var "x"))))
                              ]
--substTests02 = test [subst (parse s) ~?= v | (s, v) <- tests]
    --where tests = [ ("(Const 4) (VInt 5) "x"", 3)]
...
fromJust' (Just (VRight a)) = VRight a
fromJust' (Just (VLeft a)) = VLeft a
fromJust' (Just (VPair a1 a2)) = VPair a1 a2
fromJust' (Just (VFun n t)) = VFun n t
...
eval (Snd t) = case eval t of
                ...
                _ ->Nothing
eval (Var n) = Nothing
eval (InLeft t) = Just (VLeft $ fromJust' (eval t))
eval (InRight t)= Just (VRight $ fromJust' (eval t))
eval (Lam n t) = Just (VFun n t)
eval (App t1 t2) =
    eval t1 `thenDo` \v1 ->
    eval t2 `thenDo` \v2 ->
        case v1 of
          VFun x t -> eval (subst t v2 x)
          _        -> Nothing
                  {--
                      case (eval t1) of
                      Just (VFun n t) -> eval $ subst t (fromJust' $ eval t2) n
                      _ -> Nothing
                      --}
eval (Case t1 n1 t2 n2 t3) = case (eval t1) of
                                  Just (VLeft v) -> eval $ subst t2 v n1
                                  Just (VRight v) ->eval $ subst t3 v n2
                                  _ -> Nothing
...
A similar problem occurs in the typing of sum values.  Consider the term

    Left 1

We know that this term has type `Sum Int T` for some type `T`... but because we
only see the left constructor we don't know what the right-hand type is.

Our solution to this is to introduce variables when checking types, to be filled
in later as we continue to explore the term.  The process of filling these
...
variables as we check a term.  Type environments map term variables to
types---these track the types of variables that appear in terms (like the x in
`\x.t`).  While these types appear similar, we give them different names to
distinguish their two purposes.
...
apply [] (TVar n2) = (TVar n2)
apply [] (TInt) = (TInt)
apply [] (TPair t1 t2) = (TPair t1 t2)
apply [] (TSum t1 t2) = (TSum t1 t2)
apply [] (TFun t1 t2) = (TFun t1 t2)
apply (x:xs) (TVar n2) = if (fst x) == n2
                           then snd x
                           else apply xs (TVar n2)
apply _ (TInt) = TInt
apply x (TPair t1 t2) = TPair (apply x t1) (apply x t2)
apply x (TSum t1 t2) = TSum (apply x t1) (apply x t2)
apply x (TFun t1 t2) = TFun (apply x t1) (apply x t2)
...
structured as a function that takes four arguments:
 ...
 - An environment, giving types to the variables in the term
 - A term, to be typed
 - An excepted type for the term
 - A list of names, to be used in making up new types

and returns Just a substitution if the term can be typed, or Nothing otherwise.

Each case of the type checker will have to do two things.  It will have to make
sure that the expected type matches the type of the term---for example, a Pair
term can never produce a value of type TInt.  This will be done by unification.
Second, it will have to make sure that the subterms have the correct types.
...
check :: Environment -> Term -> Type -> Names -> Maybe TSubst
check g (Const t) ty (z:zs) = Just ((z,TInt):[])
check g (Plus t1 t2) ty (x : y : zs) =
    unify (TSum ty1 ty2) ty `thenDo` \s ->
...
check g (Pair t1 t2) ty (x : y : zs) =
    unify (TPair ty1 ty2) ty `thenDo` \s ->
    check (applyEnv s g) t1 ty1 xs `thenDo` \s' ->
    check (applyEnv s' (applyEnv s g)) t2 ty2 ys `thenDo` \s'' ->
        Just (s'' ++ s' ++ s)
    where (xs, ys) = split zs
...
-- For example, yo ushould be able to call
--
