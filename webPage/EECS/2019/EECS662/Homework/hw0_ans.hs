...
run :: [Cmd] -> [Int] -> [Int]
run cs xs = foldl (flip step) xs cs
...
compile (Plus e1 e2) = cs1 ++ cs2 ++ [Add]
    where cs1 = compile e1
          cs2 = compile e2
compile (Minus e1 e2) = cs1 ++ cs2 ++ [Subt]
    where cs1 = compile e1
          cs2 = compile e2
compile (Times e1 e2) = cs1 ++ cs2 ++ [Mult]
    where cs1 = compile e1
          cs2 = compile e2
compile (Square e) = cs ++ [Dup, Mult]
    where cs = compile e
...
decompileTests = test [ toAndFro [Push 5, Push 4, Dup, Mult, Add]
                      ...
                      , toAndFro [Push 1, Push 2, Push 3, Push 4, Add, Mult, Mult]
                      ]
    where toAndFro cs = head (run (compile (decompile cs)) []) ~?= head (run cs [])
