...
for example, a fair coin flip could be represented by the distribution
    ...
    [(1%2, VInt 0), (1%2, VInt 1)]--> Distribution [(Ratio,VInt 0)]
...
among x = among' x
    where among' [] = []
          among' (y:ys) = (1%z,y):among' ys
          z = length x
--among as = error "Unimplemented: among"

-- `certainly` is a special case of `among`, when you only have one possible
-- value.
certainly :: a -> Distribution a
certainly a = among [a]
--certainly a = error "Unimplemented: certainly"
...
impossible = []
--impossible = error "Unimplemented: impossible"
...
andThen a f = case a of
                (x:xs)->
                  let probs = map fst a in
                    let mapped = zip probs (map f (map snd a)) in
                      concat (map (\(p,qs) -> zip (map (\q->p * (fst q)) qs) (map (\q ->snd q) qs)) mapped)
                _ -> impossible
--andThen as f = error "Unimplemented: andThen"
...
binOp :: (Int -> Int -> Int) -> Environment -> Term -> Term -> Distribution Value
binOp f h t1 t2 =
...
helpEvir :: Environment -> Name -> Distribution Value
helpEvir [] _ = impossible
helpEvir ((n1,v):h) n2 = if n1 == n2 then certainly v else helpEvir h n2


eval :: Environment -> Term -> Distribution Value
eval h (Const x) = certainly (VInt x)
eval h (Plus (t1) (t2)) = binOp (+) h t1 t2
eval h (Times (t1) (t2)) = binOp (*) h t1 t2
eval h (If Zero t1 t2 t3) =
  eval h t1 `andThen` \v1 ->
  case v1 of
    VInt 0 -> eval h t2
    VInt _ -> eval h t3
eval h (If NZ t1 t2 t3) = eval h (If Zero t1 t3 t2)
eval h (If Pos t1 t2 t3) =
  eval h t1 `andThen` \v1 ->
  case v1 of
    VInt t -> if t>0 then eval h t2 else eval h t3
eval h (If NPos t1 t2 t3) =
  eval h t1 `andThen` \v1 ->
  case v1 of
    VInt t -> if t<=0 then eval h t2 else eval h t3
eval h (If Neg t1 t2 t3) =
  eval h t1 `andThen` \v1 ->
  case v1 of
    VInt t -> if t<0 then eval h t2 else eval h t3
eval h (If NNeg t1 t2 t3) =
  eval h t1 `andThen` \v1 ->
  case v1 of
    VInt t -> if t>=0 then eval h t2 else eval h t3
eval h (Var n) = helpEvir h n
eval h (Lam n t) = certainly (VFun h n t)
eval h (App t1 t2) =
  eval h t1 `andThen` \v1 ->
  eval h t2 `andThen` \v2 ->
  case v1 of
...
eval h (Let x t1 t2) =
  eval h t1 `andThen` \v ->
  eval ((x, v) : h) t2
eval h (Fix t) = eval h (Lam "$" (App (App t (Fix t)) (Var "$")))
--eval h (Between (Const x) (Const y)) = among [VInt v | v <- [x..y]]
eval h (Between t1 t2) =
  eval h t1 `andThen` \v1 ->
  eval h t2 `andThen` \v2 ->
  case (v1, v2) of
    (VInt z1,VInt z2) -> among [VInt v| v<-[z1..z2]]
eval h (Assert NZ t) =
  eval h t `andThen` \x ->
  case x of
    VInt 0 -> eval h t
    _ -> impossible
tests = runTestTT (test $ concat [distTests, regularTests])
...
stay = error "Unimplemented: Monte Hall, staying put"
switch = error "Unimplemented: Monte Hall, switching"
