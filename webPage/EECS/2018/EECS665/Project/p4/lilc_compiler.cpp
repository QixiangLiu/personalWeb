...
void
LILC::LilC_Compiler::nameAnalysis( const char * const infile, const char * const outfile ) {
	this->parse(infile);
	delete( symbolTable);
	symbolTable = new SymbolTable(); // here: new a empty symbolTable
	bool successor = this->astRoot->nameAnalysis(symbolTable);
	std::ofstream out(outfile); //open file
  if(successor){
