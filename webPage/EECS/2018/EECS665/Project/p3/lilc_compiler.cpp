...
//Important here to begin how to print nodes
// parse tree 
void
LILC::LilC_Compiler::parse( const char * const filename, const char * const outfile )
{
   ...
   std::ofstream out(outfile); //open outfile

   delete(scanner); //clear
   scanner = new LILC::LilC_Scanner( &in_stream );//in lilc_scanner.hpp
   delete(parser); //clear
   delete(astRoot); //clear reset
   try
   {
     //create a new Parser in lilc_compiler.cpp
      parser = new LILC::LilC_Parser( (*scanner) /* scanner input now scanner*/,
                                  (*this) /* compiler output file */ );
   }
   ...
   this->astRoot->unparse(out, 0); //print all good tree
   return;
}
